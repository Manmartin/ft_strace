...
syscall_t get_syscall_data64(int identifier) {
    ...
    return syscall_table64[identifier];
}
...
syscall_t get_syscall_data32(int identifier) {
    ...
    return syscall_table32[identifier];
}

static void print_syscall64(struct user_regs_struct64 *regs) {
    uint64_t    syscall_number = regs->orig_rax;
    uint32_t    argc           = get_syscall_data64(syscall_number).argc;
    const char *name           = get_syscall_data64(syscall_number).name;
...
static void print_syscall32(struct user_regs_struct32 *regs) {
    ...
    uint32_t    argc           = get_syscall_data32(syscall_number).argc;
    const char *name           = get_syscall_data32(syscall_number).name;
